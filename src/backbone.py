...
from keras.layers import MaxPooling2D, AveragePooling2D, GlobalAveragePooling2D, TimeDistributed, Bidirectional, GRU, LSTM
...
def resnet_2D_v1(input_dim, mode='train'):
    ...
    x1 = conv_block_2D(x1, 3, [32, 64, 64], stage=5, block='new_a', trainable=True)
    x1 = conv_block_2D(x1, 3, [64, 128, 128], stage=5, block='new_b', trainable=True)
    x1 = conv_block_2D(x1, 3, [128, 128, 128], stage=5, block='new_c', trainable=True)
    x1 = conv_block_2D(x1, 3, [128, 64, 64], stage=6, block='new_d', trainable=True)
    x1 = conv_block_2D(x1, 3, [64, 64, 32], stage=6, block='new_e', trainable=True)
    x1 = conv_block_2D(x1, 3, [32, 16, 16], stage=6, block='new_f', trainable=True)
    # x1 = conv_block_2D(x1, 3, [32, 16, 16], stage=6, block='g', trainable=True)
    # x1 = conv_block_2D(x1, 3, [32, 16, 16], stage=6, block='h', trainable=True)

    x1 = Flatten()(x1)

    x1 = models.Model(inputs, x1)

    model = models.Sequential()
    model.add(TimeDistributed(x1, input_shape=(None,)+input_dim))
